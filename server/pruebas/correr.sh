...
correr() {
  echo ""
  echo "── $1 ──"
  rm -f prueba.db ap.db vig.db mig.db fijos.db presu.db det.db
...
correr t-detectar.ts

correr t-migracion.ts

# Éstas levantan su propio servidor y hablan con la API como dos personas.
rm -f privacidad.db personal.db fijos.db periodo.db reparto.db arr.db cta.db cat.db cita.db atajo.db
